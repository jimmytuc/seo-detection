 ...
 * @return {Array<String>}
 */
export function detectMetaTags(tags, metaRules) {
    ...
    let filterNotFoundName = filter(rule => (
        indexOf(
            rule, 
            tags.map(item => item.getAttribute('name'))
        ) == -1
    ), metaRules);

    let getTagsFoundName = filter(tag => (
        indexOf(
    ...
    let filterNotFoundContent = filter(tag => {
        let content = tag.getAttribute('content');
        return (typeof content === 'undefined' || !content);
    }, getTagsFoundName);
    return concat(
        map(item => `Not found meta ${item} in this HTML`, filterNotFoundName),
        map(item => `Meta ${item.getAttribute('name')} missing content in this HTML`, filterNotFoundContent)
    );
}
 ...
 * @return {Array<String>}
 */
export function detectTagsWithoutAttribs(tags, attribute) {
    if((tags instanceof Array) === false) {
        throw new Error('Invalid tags parameter.');
    }
 ...
 * @return {String}
 */
export function detectCountTagExccedLimit(tags, limit) {
    if((tags instanceof Array) === false) {
        throw new Error('Invalid tags parameter.');
    }
 ...
 */
export function detectRequiredTags(tags, limit, tagType) {
